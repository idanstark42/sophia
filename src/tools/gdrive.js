...
const getActions = async () => {
  ...
  return {
    get_calendar_events: {
      ...
      postProcessing: events => {
        events.forEach(event => {
          event.type = COLORS_CODING[event.color]
    ...
    add_calendar_event: {
      params: { calendarName: { type: 'string', enum: calendarNames }, title: { type: 'string' }, startDate: { type: 'date' }, endDate: { type: 'date' }, description: { type: 'string' }, allDay: { type: 'boolean' }, type: { type: 'string', enum: Object.values(COLORS_CODING) } },
      preProcessing: params => {
        params.start = params.startDate
        params.end = params.endDate
        delete params.startDate
        delete params.endDate
        params.color = Object.entries(COLORS_CODING).find(([_, type]) => type === params.type)[0]
        return params
      }
    },
  }
